...
watch(
    params,
    debounce(() => {
        router.get(route('sale-return-payments.index', props.saleReturn.id), pickBy(params), {
            preserveScroll: true,
            preserveState: true,
        });
    }, 300),
...
const form = useForm({
    sale_return_id: props.saleReturn.id,
...
const createModal = () => {
    ...
    form.reset();
    form.sale_return_id = props.saleReturn.id;
...
const editModal = (id) => {
    ...
    fetch(route('sale-return-payments.edit', id), {
        method: 'GET',
        headers: {
        ...
        .then((data) => {
            payment.value = data;
            form.sale_return_id = data.sale_return_id;
...
const store = () => {
    if (!modalState.editing) {
        form.post(route('sale-return-payments.store'), {
            preserveScroll: true,
            onSuccess: () => {
                let { Toast } = useAlert();
        ...
        form.put(route('sale-return-payments.update', payment.value.id), {
            preserveScroll: true,
            onSuccess: () => {
                let { Toast } = useAlert();
...
const deletePayment = (payment) => {
    ...
    AlertWithButtons.fire({
        ...
        if (result.isConfirmed) {
            router.delete(route('sale-return-payments.destroy', payment), {
                preserveScroll: true,
                onSuccess: () => {
                    Toast.fire({
                        icon: 'warning',
                        title: 'Refund Payment Deleted!',
                    });
                },
                onError: () => {
                    Toast.fire({
...
// Modal Title
const modalTitle = computed(() => (modalState.editing ? 'Edit Refund Payment' : 'Add Refund Payment'));
</script>

<template>
    <Head title="Sale Return Payments" />
    ...
    <Breadcrumb>
        ...
        <li class="breadcrumb-item">
            <Link :href="route('sale-returns.show', saleReturn.id)">{{ saleReturn.reference }}</Link>
        </li>
        <li class="breadcrumb-item active" aria-current="page">Refund Payments</li>
    </Breadcrumb>
    <!-- Breadcrumb End -->

    <!-- Page Content Start -->
    <div class="row">
        <!-- Sale Return Summary Card -->
        <div class="col-12 mb-4">
            <div class="card border-0 shadow">
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-3">
                            <h6 class="text-muted mb-1">Return Reference</h6>
                            <p class="fw-bold mb-0">{{ saleReturn.reference }}</p>
                        </div>
                        <div class="col-md-3">
                            <h6 class="text-muted mb-1">Customer</h6>
                            <p class="fw-bold mb-0">{{ saleReturn.customer.name }}</p>
                            <p class="mb-0 text-muted">{{ saleReturn.customer.mobile ?? '-----' }}</p>
                        </div>
                        <div class="col-md-2">
                            <h6 class="text-muted mb-1">Total Amount</h6>
                            <p class="fw-bold mb-0">{{ formatCurrency(saleReturn.total) }}</p>
                        </div>
                        <div class="col-md-2">
                            <h6 class="text-muted mb-1">Refunded</h6>
                            <p class="fw-bold mb-0 text-success">{{ formatCurrency(saleReturn.refunded) }}</p>
                        </div>
                        <div class="col-md-2">
                    ...
                    <div class="row">
                        <div class="col-md-8 mb-3 mb-md-0">
                            <button @click="createModal" class="btn btn-primary">
                                Add Refund Payment <i class="ri-add-line"></i>
                            </button>
                        </div>
